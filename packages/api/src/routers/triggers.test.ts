...
describe("work that starts because something happened", () => {
  it("raises the Instance a Move calls for, on the day the Playbook says, for the animal it happened to", async () => {
    ...
    expect(mine[0]?.dueAt.toISOString()).toBe("2026-10-04T02:00:00.000Z");
  ...
  it("raises it once however often the app is opened, and once more for a second Move", async () => {
    ...
    // Other test files share this database and move their own animals about, so this is
    // asserted on this SOP's own Pens rather than on everything the farm has late.
    const all = await owner.client.instances.overdue();
    ...
    expect(
      all
        ...
        .toSorted()
    ).toEqual(["2026-10-04T02:00:00.000Z"]);
  });
});
...
describe("what may be triggered on", () => {
  it("refuses to publish a Trigger the farm has no way of noticing", async () => {
    ...
    ).rejects.toThrow(/"grumpy" is not a State an animal has/u);
  });
});
