...
/** A year of its own: a sweep of this farm's work must not reach into another file's (the shared test farm). */
...
describe("the day turning", () => {
  ...
  it("carries no post while the farm is asleep", async () => {
    ...
    expect(night.post.people).toBe(0);
    expect(night.wentWrong).toEqual([]);
  });

  it("turns the rest of the day when one piece of it fails", async () => {
    const context = await buildContext({
      session: null,
    ...
    // A farm whose quiet hours are nonsense: reading them throws where the post is worked out.
    const turned = await theDayTurns({
      ...context,
      farm: { ...farm, quietFrom: null as unknown as string },
    } as Turning);

    // The week's work was still raised, and what went wrong is said rather than thrown.
    expect(turned.workRaised).toBeGreaterThan(0);
    expect(turned.wentWrong.some((one) => one.startsWith("the post"))).toBe(
      true
    );
  });
});
