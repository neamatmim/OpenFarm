...
**The finishing set** (18 tickets, 2026-09-18/20) closed what the increments left: Adjustments lifted out of the settlement store, a settled Venture refusing the Corrections that would move its figures, a Correction that reaches every Venture it touches, and the Settlement, its payouts and its Adjustments on screen. Tickets 07–18 followed: two Ventures in the demo seed, the Owner's farm page and the Manager's saying what they run, a Selling Trip that somebody pays for, signing without the photo, the paisa a settled account keeps, an Agreement that may be amended, shed hygiene staying the Farm's, the default split as a Farm Parameter, Reimbursed as a figure of its own, a Venture's margin and cost of gain, and records kept on purpose.
...
**Caveat worth carrying.** Most of these screens were shipped without anybody looking at them. The first one that was opened — the statements sheet, 2026-09-19 — turned up three defects in a sitting, none of them in the code under review: a column header over no rows, a notice printing its own `{placeholders}`, and a month rendered `2026-09` inside a Bangla sentence. The Settlement, payout and Adjustment screens were opened for the first time on 2026-09-19, once the seed made a Venture to look at. **On 2026-09-20 the rest were opened**, while the Investors and the Ventures pages were redrawn on the page kit: every act on a Venture's card, in both languages and in every state the seed holds — movements, sign, take capital, call off, draw and count the Float, reimburse, bank check, advance, the buy-back, settlement, economics, statements, amend, the Internal Sale and opening a Venture — was clicked and opens its own sheet. Two defects fell out, both of the kind a screenshot finds and a test does not: a balance carrying its paisa ran off the side of its tile (`StatTile` wraps now, for every page), and the bank badge said `2026-08` inside a Bangla sentence (`saidMonth` in `apps/web/src/lib/months.ts` words it, and the Owner's farm page had the same line).

**The two Venture screens are of a piece with the rest of the app** (2026-09-20). They were the only working pages not built from `components/page-kit.tsx`. The Ventures page now opens on four figures and three tabs kept in the address (`?tab=running|settled|cancelled`, Open counting as running), and a card — lifted into `components/ventures/venture-card.tsx` — shows the figures that matter where the run stands, says its terms once quietly, and draws at most two buttons with every other act in the menu beside its name. The Investors page gained the same figures, a search by name or phone, and a sheet reading back the NID and bank account it asks for and had never shown again.
...
- [Take the structure to a lawyer and a Shariah scholar](./issues/11-take-the-structure-to-a-lawyer-and-a-shariah-scholar.md) — written opinions in hand (2026-09-26). Both advisers approved the structure and every draft as they stand, with no changes needed. The permissions asked for things not built (Master Agreement and Schedule, e-challan, e-signatures, stamping on request, paying in the portal) are not taken up.
- [What the advisers' answers change](./issues/12-what-the-advisers-answers-change.md) — nothing: the structure, the cap, the Agreement, the software, the Shariah charges and every earlier decision stand.

## Not yet specified

- **Tax and the accountant.** How project money appears in the monthly accountant export, and what the software must withhold or show. The research found 10% withholding if a contract reads as a deposit, and none found for a genuine profit share; VAT is likely on any fee. The advisers raised nothing the software must withhold or show (2026-09-26), so this waits only on how investor money reaches the accountant export.
