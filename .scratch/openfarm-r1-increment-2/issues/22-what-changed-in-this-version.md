...
- Times are in the reader's digits; the notice's key matches the naming of every other key; the cheap question is asked first, so most board loads read one row instead of three; and the glossary's **Alert** entry now says what makes one an Alert — that it goes *now* — since the in-app list carries the quieter notices beside them.
