...
- **Where else the Nominees show.** The portal's account page, the Data Copy, the যোগদানপত্র, the Investor sheet and table, and the audit words. Now that 03 has given each Nominee a share, a date of birth and perhaps a Receiver, this is a small table rather than one line. It is expected to be spec detail, drawn the way the prototype lays out the paper.
- **Consent for each Nominee's data.** The Receiver now signs for a minor Nominee's details (03). What is left is whether the **Portal Consent** sheet's and the data clause's "nominee details" need new words for dates of birth and Receivers. That is for the prototype or the spec, and the lawyer sees it on the page.
