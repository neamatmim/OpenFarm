...
**One Investor may hold two shares on one Venture.** `venture_settlement_share` is unique on `(settlementId, agreementId)`, not on the Investor, and the schema says why at `packages/db/src/schema/venture.ts:372` — "the same person may hold two papers on one Venture". So narrowing by `investorId` can return more than one row, and this paper has to decide whether a man with two Agreements gets one sheet showing both or a sheet per Agreement. Each Agreement froze its own percentages and its own Target Window, so they are not always addable; the safer reading is a sheet per Agreement, and either way a paper that silently showed only the first would short-change him.
