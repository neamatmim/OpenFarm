...
**Template**: The farm's wording for one kind of paper an Investor signs — the **Investment Agreement**, the **Master Agreement**, the **Venture Schedule** and the **Amendment** — laid out in parts (the parties, the facts, numbered clauses, the stamp, the signatures), each in Bangla with the English beside it, with the paper's own facts asked for by name and filled when it is printed. One per kind, starting as OpenFarm's standard wording for the farm's lawyer to read. The Owner alone changes it, by publishing its next Version; a Version is never rewritten, because every paper records the Version it was signed in and the farm must print it again as it was signed. A lawyer's approval is written onto one Version, once, with the lawyer's name and the day, and never carried to the next. The letterhead, who the parties are, the stamp's blanks and the closing lines promising no return are the farm's, not the wording's. Not the **SOP Definition**, which words work, not a paper. _Avoid_: Form, boilerplate, contract template

**Master Agreement**: One stamped mudarabah agreement between the Owner and one Investor, setting the terms every **Venture** they join shares; each Venture is then joined by a **Venture Schedule**. The shape the Owner asked the lawyer about so that each Venture needs no stamp paper of its own. Worded and printable as a **Template**; nobody signs one until the lawyer answers (the investor map's ticket 11), and until then every Venture is signed with its own **Investment Agreement**. _Avoid_: Framework agreement, umbrella agreement

**Venture Schedule**: The short paper joining one Investor to one **Venture** under their **Master Agreement**: the Units, the capital, the split and the **Target Window**. Waits on the lawyer with the Master Agreement. _Avoid_: Annex, addendum (an **Amendment** is what changes a paper already signed)
