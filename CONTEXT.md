...
**Investor Portal**: Where an Investor the Owner has invited reads the Ventures they are in, how each is doing today, and their own three **Investor Statements** — read-only, nothing taken, paid, signed or offered through it (ADR 0007). By invitation, one person at a time: the Owner hands over a one-time code, and the Investor chooses their own password with their phone number and the code. Their account holds no Role and is not a **Membership**; the Owner takes it away, and the whole portal is closed until the Owner opens it — which is how the farm answers a lawyer who says it is a platform. The Owner's decision of 2026-09-24, taken before the lawyer answered. _Avoid_: Investor login, investor app, dashboard
