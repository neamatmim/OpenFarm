...
export function PrototypeSwitcher({ variants, current }: Props) {
  ...
  const go = (delta: number) => {
    ...
    if (!next) return;
    navigate({ to: ".", search: (prev: Record<string, unknown>) => ({ ...prev, variant: next.key }) });
  };
  ...
  return (
    <div className="fixed bottom-3 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full border-2 border-yellow-400 bg-black/90 px-3 py-1.5 font-mono text-xs text-yellow-300 shadow-xl">
      <button type="button" onClick={() => go(-1)} aria-label="previous variant" className="p-1">
        <ChevronLeft size={16} />
      </button>
      <span>
        PROTOTYPE {variants[idx]?.key} — {variants[idx]?.name}
      </span>
      <button type="button" onClick={() => go(1)} aria-label="next variant" className="p-1">
