...
Decided on the wayfinder map: `.scratch/openfarm-release-1/issues/17-offline-capture-and-sync-decision.md`; evidence on branch `research/offline-capture`. See also ADR 0004: each Entry is one module both the procedure and the batch call, and the farm record is dated by when it was done.
